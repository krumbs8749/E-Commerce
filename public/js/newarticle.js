...
function createForm() {
    ...
    form.onsubmit = (event) => {
        ...
        if (formValues["art_price"].value <= 0) {
            ...
            console.log({
                name: formValues["art_name"].value,
                price: formValues["art_price"].value,
                desc: formValues["art_description"].value,
            });
        }
        sendData(formValues);
...
function sendData(fD) {
    ...
    xhr.setRequestHeader(
        "X-CSRF-Token",
        document.getElementById("script-newarticle").dataset.token
    );
    // Handle Webserver Response
    let formData = new FormData();
    formData.append("art_name", fD["art_name"].value);
    formData.append("art_price", fD["art_price"].value);
    formData.append("art_description", fD["art_description"].value);
